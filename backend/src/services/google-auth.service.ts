...
export async function verifyGoogleIdToken(idToken: string) {
  ...
  if (!audience) throw Object.assign(new Error("Google OAuth not configured"), { status: 503 });

  const ticket = await client.verifyIdToken({ idToken, audience });
  const payload = ticket.getPayload();
  if (!payload?.email) throw Object.assign(new Error("Invalid Google token"), { status: 401 });
...
export async function findOrCreateGoogleUser(
  input: { googleId: string; email: string; name: string },
