...
analyticsUserRouter.get("/student", requireAuth, requireRole(["STUDENT"]), asyncHandler(async (req, res) => {
    const summary = await studentAnalyticsSummary(req.auth.userId);
    res.json(summary);
}));
analyticsUserRouter.get("/mentor", requireAuth, requireRole(["MENTOR"]), asyncHandler(async (req, res) => {
