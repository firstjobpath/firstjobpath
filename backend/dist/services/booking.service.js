...
export async function listAvailableSlots(mentorId, from, to) {
...
export async function createBookingWithSlot(input) {
    ...
    if (slot.seatsTaken >= slot.seatsTotal) {
        const pos = (await prisma.bookingWaitlist.count({
        ...
        return { waitlist: true, position: pos };
    }
